...
/**
 * 每一项都是针对公共查询维度的必要条件，而不是原文全部资格的复刻；所有已记录条件匹配或条件为空，也不证明某账户最终适用。同一字段在一项权益中只出现一次，one_of 可局部保留来源冲突的候选值。model 值仅记录该渠道优惠适用条款直接列明的称谓或 ID，同一条款列出多个 ID 时全部保留，不从另行说明的路由或兼容关系推导更多值。账户历史、实际发放、效果解释和一般说明不得写入此处。
 *
...
/**
 * 一份文件记录同一计费渠道下的一项独立时段权益，而不是笼统对应一张营销活动页面。仅收录通用语言模型及直接影响其使用成本或额度的套餐，不收录专用媒体生成模型、结构化决策模型或独立工具、安全、运行时服务。全部适用情形共享同一政策触发事件和效果范围；套餐档位、计费项或事件导致实际效果不同时拆成不同权益。
 */
